...
import type { OnchainStoreCartReact } from '../lib/types';
...
export default function OnchainStoreCart({
  ...
  //     .map((productId) => `${productId}(${quantities[productId]})`)
  //     .join(',');
  //   const chargeDetails = {
  //     name: 'commerce template charge',
  //     description,
  //     pricing_type: 'fixed_price',
  //     local_price: { amount: totalSum.toString(), currency: 'USD' },
  //   };
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-col items-start justify-between py-4 md:flex-row md:items-center">
          ...
          <div className="flex w-full grow flex-col items-center justify-between gap-2 px-4 sm:flex-row sm:gap-0 md:w-auto lg:px-6">
            ...
            <div className="w-64">
              {/* TODO: comment back in to enable checkout flow */}
              {/* <Checkout key={key} onStatus={handleStatusChange} chargeHandler={chargeHandler}>
                <CheckoutButton coinbaseBranded={true} text="Pay with Crypto" disabled={!totalSum} />
              </Checkout> */}
